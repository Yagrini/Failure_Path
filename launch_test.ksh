...
#"-Wl,-emainRTRT"
RM="rm -f"
TENIS="/home_pers/ratsiamb/Documents/INSA_TENIS/TENIS/TENIS/bin/linux/tenis"
TENIS_POST_PRO="/home_pers/ratsiamb/Documents/INSA_TENIS/TENIS/TENIS_POSTPRO/bin/linux/tenis_postpro"
TEST_PTU="test/Failure_Path.ptu"
...
if [[ ( "$1" == "NO_TEST" ) || ( $1 -eq '1' ) ]]
    then
    clean_all_useless_file
    compile src/main.c src/Failure_Path.c
    bin/prog
...
then
   ...
   # sed -i 's/end/end\nrun/4' /tmp/user_debug.gdb	
    compile /tmp/maintenis_tmp.c src/Failure_Path.c
    run_TENIS_script_GDB
    ${TENIS_POST_PRO} test_result.log
    ${READER} rapport_test.txt

    ## Lance un test en utilisant TENIS avec couverture de code du fichier src/math.c
elif [[ ( "$1" == "COUV_math" ) || ( $1 -eq '5' ) ]]
then
    clean_all_useless_file
    generate_prepro_file src/Failure_Path.c
    ${TENIS} ${TEST_PTU} -c src/Failure_Path.c.out
    compile /tmp/maintenis_tmp.c src/Failure_Path.c
...
then
    ...
    echo "5) COUV_math.c: test avec couverture de code de src/math.c"
